write_cleanfile_csv = function(dat) {
    ...
    answer <- readline(prompt = "Write clean file as xlsx? Y/N: ")
    ...
    if (toupper(answer) == "Y") {
        ...
        filename <- readline(prompt = "Enter the filename (with .xlsx extensiopn): ")
        ...
        # save file
        openxlsx::write.xlsx(dat, file = full_path, row.names = FALSE)
        ...
        cat("\n\nRecommendaton: Write the output as .xlsx and review if the input
            is in cleaned state.\n")
